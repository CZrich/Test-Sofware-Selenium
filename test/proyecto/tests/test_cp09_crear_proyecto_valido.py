...
def test_cp09_crear_proyecto_valido(driver):
    ...
    nombre_input.clear()
    nombre_input.send_keys("Proyecto A")

    # Seleccionar departamento
    depto_select = wait.until(EC.presence_of_element_located((By.XPATH, "//select[contains(@class, 'rounded-md') and @required]")))
    Select(depto_select).select_by_index(1)  # Selecciona el primer departamento disponible

    # Inputs de fecha (por orden en el formulario)
    ...
    wait.until(EC.text_to_be_present_in_element(
        (By.XPATH, "//div[contains(@class,'divide-y')]"), "Proyecto A"
    ))
    save_screenshot(driver, "cp09_crear_proyecto_valido")